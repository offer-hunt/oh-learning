...
public class AiAssistanceService {
    ...
    public AiResponseDto explainMaterial(UUID userId, ExplainRequestDto req) {
        if (!StringUtils.hasText(req.getUserQuery())) {
            log.warn("{User ID: {}} requested AI explanation for {Materials ID: {}}. Status: Empty Request.",
                    userId, req.getMaterialId());
        ...
        try {
            AiExternalExplainRequest aiReq = AiExternalExplainRequest.builder()
            ...
            log.info("{User ID: {}} requested AI explanation for {Materials ID: {}}. Request: \"{}\". Status: Success.",
                    userId, req.getMaterialId(), req.getUserQuery());

            return response;

        } catch (Exception e) {
            log.error("{User ID: {}} requested AI explanation for {Materials ID: {}}. Status: AI Service Unavailable",
                    userId, req.getMaterialId());
            return AiResponseDto.builder()
                    .success(false)
    ...
    public AiResponseDto getTaskHint(UUID userId, UUID taskId, HintRequestDto req) {
        ...
        AiTaskStats stats = aiStatsRepo.findById(id).orElseGet(() -> {
        ...
        if (stats.getHintsUsed() >= MAX_HINTS) {
            log.warn("{User ID: {}} requested AI hint for task {Task ID: {}}. Hints used: {}. Status: Limit Exceeded.",
                    userId, taskId, MAX_HINTS);
            return AiResponseDto.builder()
                    .success(false)
        ...
        try {
            AiExternalHintRequest aiReq = AiExternalHintRequest.builder()
                    .taskId(taskId)
            ...
            if (response.isSuccess()) {
                ...
                log.info("{User ID: {}} requested AI hint for task {Task ID: {}}. Hints used: {}. Status: Success.",
                        userId, taskId, stats.getHintsUsed());
            }

            return response;

        } catch (Exception e) {
            log.error("{User ID: {}} requested AI hint for task {Task ID: {}}. Status: AI Service Unavailable.",
                    userId, taskId);
            return AiResponseDto.builder()
                    .success(false)
